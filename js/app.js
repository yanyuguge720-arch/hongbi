...
function getDailyUsage() {
  ...
  return d.date === getTodayStr() ? (d.count||0) : 0;
}
function incrementUsage() {
  const d = JSON.parse(localStorage.getItem(USAGE_KEY) || '{}');
  d.date = getTodayStr(); d.count = (d.count||0)+1;
  localStorage.setItem(USAGE_KEY, JSON.stringify(d));
}
function isPro() {
  const p = JSON.parse(localStorage.getItem(PRO_KEY) || '{}');
  return p.active && new Date(p.expiry||0) > new Date();
}
function setPro(days) {
  const d = new Date(); d.setDate(d.getDate()+(days||30));
  localStorage.setItem(PRO_KEY, JSON.stringify({active:true, expiry:d.toISOString(), activated:Date.now()}));
}
function checkUsageThen(fn) {
  if (isPro() || getDailyUsage() < DAILY_LIMIT) {
    fn();
...
async function exportImage() {
  ...
  const rendered = renderMarkdown(formatted);
  if (!rendered) { showToast('\u6ca1\u6709\u5185\u5bb9\u53ef\u5bfc\u51fa'); return; }
  const W=1080,H=1440, lh=lineHeightValue(currentSpacing);
  const align = currentAlign==='left'?'start':currentAlign==='center'?'center':'end';
  const style = `*{margin:0;padding:0;box-sizing:border-box;}
    body{width:${W}px;height:${H}px;font-family:"PingFang SC","Microsoft YaHei",sans-serif;
    padding:56px 48px;background:white;line-height:${lh};text-align:${align};
    color:#2c2c2c;font-size:28px;word-wrap:break-word;position:relative;}
    .bar{position:absolute;top:0;left:0;right:0;height:6px;background:linear-gradient(90deg,#E84C4C,#d63031);}
    strong{font-weight:700;} em{font-style:italic;} del{text-decoration:line-through;color:#999;}
    .tag{color:#E84C4C;font-weight:600;}
    .ft{position:absolute;left:0;right:0;bottom:32px;text-align:center;font-size:18px;color:#ccc;}`;
  const footer = '<div class="ft">\u2014\u2014 \u7ea2\u7b14 \u00b7 \u8ba9\u4f60\u7684\u7b14\u8bb0\u66f4\u597d\u770b \u2014\u2014</div>';
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>${style}</style></head><body><div class="bar"></div>${rendered}${footer}</body></html>`;
  try {
    const img = new Image();
    ...
    c.toBlob(b => {
      ...
      a.download='hongbi_'+Date.now()+'.png'; a.click();
...
$('clearFormatBtn').addEventListener('click', () => {
  let t = inputArea.value;
  const b = t;
  t = t.replace(/\*\*(.+?)\*\*/g,'$1').replace(/\*(.+?)\*/g,'$1').replace(/~~(.+?)~~/g,'$1').replace(/#/g,'');
  if (t===b) { showToast('\u6ca1\u6709\u683c\u5f0f\u9700\u8981\u6e05\u9664'); return; }
  inputArea.value = t; updatePreview(); pushHistory(t);
  showToast('\u5df2\u6e05\u9664\u6240\u6709\u683c\u5f0f');
});
$('italicBtn').addEventListener('click', () => wrapSelection('*','*','\u659c\u4f53\u6587\u5b57'));
$('strikeBtn').addEventListener('click', () => wrapSelection('~~','~~','\u5220\u9664\u7ebf'));
...
document.querySelectorAll('.spacing-btn').forEach(b => {
  b.addEventListener('click', () => {
    document.querySelectorAll('.spacing-btn').forEach(x=>x.classList.remove('active'));
    b.classList.add('active'); currentSpacing = b.dataset.spacing; updatePreview();
  });
});
document.querySelectorAll('.align-btn').forEach(b => {
  b.addEventListener('click', () => {
    document.querySelectorAll('.align-btn').forEach(x=>x.classList.remove('active'));
    b.classList.add('active'); currentAlign = b.dataset.align; updatePreview();
  });
});

const usageEl = document.getElementById('usageRemaining');
const usageBtn = document.getElementById('usageUpgradeBtn');
if (usageBtn) usageBtn.addEventListener('click', e => { e.preventDefault(); if(!isPro()) proModal.classList.add('show'); });
function updateUsageUI() {
  if (!usageEl) return;
  if (isPro()) {
    usageEl.textContent = '\u221e';
    if (usageBtn) { usageBtn.textContent='Pro\u5df2\u6fc0\u6d3b'; usageBtn.style.color='#4CAF50'; }
  } else {
    usageEl.textContent = Math.max(0, DAILY_LIMIT - getDailyUsage());
    if (usageBtn) { usageBtn.textContent='\u5347\u7ea7 Pro \u2192'; usageBtn.style.color=''; }
  }
}
updateUsageUI();
...
if (expBtn) expBtn.addEventListener('click', function(e) {
  ...
  checkUsageThen(() => exportImage());
}, true);

// ===== 激活码 UI =====
const actBtn = document.getElementById('proActivateBtn');
const actInput = document.getElementById('proCodeInput');
if (actBtn && actInput) {
  actBtn.addEventListener('click', () => {
    const c = actInput.value.trim();
    if (!c) { showToast('\u8bf7\u8f93\u5165\u6fc0\u6d3b\u7801'); return; }
    activatePro(c);
    updateUsageUI();
  });
  actInput.addEventListener('keydown', e => { if (e.key==='Enter') actBtn.click(); });
}

// ===== Pro 弹窗状态 =====
const pStatus = document.getElementById('proStatus');
const pPayment = document.getElementById('proPayment');
const pActivated = document.getElementById('proActivated');
function updateProModalUI() {
  if (!pStatus) return;
  if (isPro()) {
    pStatus.innerHTML = '<span class="pro-status-badge pro-status-pro">Pro\u5df2\u6fc0\u6d3b\u2713</span>';
    if (pPayment) pPayment.style.display='none';
    if (actInput) actInput.style.display='none';
    if (actBtn) actBtn.style.display='none';
    if (pActivated) { pActivated.style.display='block'; pActivated.textContent='Pro\u5df2\u6fc0\u6d3b\uff0c\u611f\u8c22\u652f\u6301\uff01'; }
  } else {
    pStatus.innerHTML = '<span class="pro-status-badge pro-status-free">\u514d\u8d39\u7248</span>';
    if (pPayment) pPayment.style.display='';
    if (actInput) actInput.style.display='';
    if (actBtn) actBtn.style.display='';
    if (pActivated) pActivated.style.display='none';
  }
}
updateProModalUI();
...
updatePreview = function() {
  const raw = inputArea.value, len = raw.length;
  const pct = Math.min((len/1000)*100,100);
  const bar = document.getElementById('charBar');
  if (bar) bar.style.width = pct+'%';
  const area = document.querySelector('.char-area');
  if (area) {
    area.className = 'char-area';
    if (len>1000) area.classList.add('exceed');
    else if (len>850) area.classList.add('warning');
  }
  ...
  if (formatted) {
    ...
    previewArea.innerHTML = '<div class="preview-placeholder">\u6392\u7248\u6548\u679c\u4f1a\u663e\u793a\u5728\u8fd9\u91cc</div>';
  }
  const t = raw.trim();
  if (statChars) statChars.textContent = t.length;
  if (statParagraphs) statParagraphs.textContent = t ? t.split('\n\n').filter(p=>p.trim()).length : 0;
  if (statReadTime) statReadTime.textContent = Math.ceil(t.length/300)+'s';
  updateUsageUI();
};
updatePreview();